...
if [[ "${NAME}" == "Amazon Linux" ]] && [[ $VERSION == 2 ]]; then
	DEPS_FILE="${REPO_ROOT}/scripts/infrablockchain_build_amazonlinux2_deps"
else
	echo " - You must be running Amazon Linux 2017.09 or higher to install InfraBlockchain." && exit 1
fi
