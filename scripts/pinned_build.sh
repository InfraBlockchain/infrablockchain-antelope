...
echo "Infrablockchain Pinned Build"
...
if [ $# -eq 0 ] || [ -z "$1" ]
   then
      echo "Please supply a directory for the build dependencies to be placed and a directory for Infrablockchain build and a value for the number of jobs to use for building."
      echo "The binary packages will be created and placed into the Infrablockchain build directory."
      echo "./pinned_build.sh <dependencies directory> <Infrablockchain build directory> <1-100>"
      exit -1
fi
...
# build Infrablockchain
echo "Building Infrablockchain ${SCRIPT_DIR}"
...
echo " "
echo "Infrablockchain has successfully built and constructed its packages.  You should be able to find the packages at ${LEAP_DIR}.  Enjoy!!!"
