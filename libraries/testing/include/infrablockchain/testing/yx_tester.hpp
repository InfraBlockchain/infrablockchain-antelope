#ifndef INFRABLOCKCHAIN_SYS_TESTER_HPP
#define INFRABLOCKCHAIN_SYS_TESTER_HPP
...
#include <infrasys/infrasys.wast.hpp>
#include <infrasys/infrasys.abi.hpp>
#include <sys.txfee/sys.txfee.wast.hpp>
#include <sys.txfee/sys.txfee.abi.hpp>
#include <sys.identity/sys.identity.wast.hpp>
#include <sys.identity/sys.identity.abi.hpp>
...
namespace infrablockchain { namespace testing {

class yx_tester : public tester {
public:
   using id_type = uint64_t;

   const string zero_ntoken = asset{0, symbol{INFRABLOCKCHAIN_NATIVE_TOKEN_SYMBOL}}.to_string();
   ...
   void init_infrablockchain_contracts() {
      produce_blocks(2);

      create_accounts({N(d1), N(user1), N(user2), INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT, INFRABLOCKCHAIN_SYS_TX_FEE_ACCOUNT,
                       INFRABLOCKCHAIN_SYS_IDENTITY_ACCOUNT, INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT, INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT,
                       INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_EXCHANGE_ACCOUNT});
      produce_blocks(2);

      set_code(config::system_account_name, infrasys_wast);
      set_abi(config::system_account_name, infrasys_abi);
      ...
      set_code(INFRABLOCKCHAIN_SYS_TX_FEE_ACCOUNT, sys_txfee_wast);
      set_abi(INFRABLOCKCHAIN_SYS_TX_FEE_ACCOUNT, sys_txfee_abi);

      produce_blocks();

      auto &accnt3 = control->db().get<account_object, by_name>(INFRABLOCKCHAIN_SYS_TX_FEE_ACCOUNT);
      ...
      set_tx_fee("tf.nredeem", asset{100000, symbol{INFRABLOCKCHAIN_NATIVE_TOKEN_SYMBOL}}.to_string());
      set_tx_fee("tf.ntransfer", asset{10000, symbol{INFRABLOCKCHAIN_NATIVE_TOKEN_SYMBOL}}.to_string());
      set_tx_fee("tf.transfer", asset{10000, symbol{INFRABLOCKCHAIN_NATIVE_TOKEN_SYMBOL}}.to_string());
      ...
      set_code(INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT, yx_ntoken_wast);
      set_abi(INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT, yx_ntoken_abi);

      produce_blocks();

      auto &accnt = control->db().get<account_object, by_name>(INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT);
      BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
      abi_ser_ntoken.set_abi(abi, abi_serializer_max_time);

      produce_blocks();

      set_code(INFRABLOCKCHAIN_SYS_IDENTITY_ACCOUNT, sys_identity_wast);
      set_abi(INFRABLOCKCHAIN_SYS_IDENTITY_ACCOUNT, sys_identity_abi);

      auto &accnt4 = control->db().get<account_object, by_name>(INFRABLOCKCHAIN_SYS_IDENTITY_ACCOUNT);
      BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt4.abi, abi), true);
      abi_ser_identity.set_abi(abi, abi_serializer_max_time);

      produce_blocks();

      set_code(INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT, yx_token_wast);
      set_abi(INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT, yx_token_abi);

      produce_blocks();

      auto &accnt_token = control->db().get<account_object, by_name>(INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT);
      ...
      set_code(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT, yx_nft_wast);
      set_abi(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT, yx_nft_abi);

      produce_blocks();

      auto &accnt_nft = control->db().get<account_object, by_name>(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT);
      ...
      set_code(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_EXCHANGE_ACCOUNT, yx_nftex_wast);
      set_abi(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_EXCHANGE_ACCOUNT, yx_nftex_abi);

      produce_blocks();

      auto &accnt_nftex = control->db().get<account_object, by_name>(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_EXCHANGE_ACCOUNT);
   ...
   string to_asset_string(share_type a, uint64_t symbol_value = INFRABLOCKCHAIN_NATIVE_TOKEN_SYMBOL) {
      return asset{a, symbol{symbol_value}}.to_string();
   }

   string to_yx_asset_string(share_type a, const string &issuer, uint64_t symbol_value = INFRABLOCKCHAIN_NATIVE_TOKEN_SYMBOL) {
   ...
   action_result push_action(const action_name &name, const variant_object &data, const account_name &signer,
                             ...
                             const account_name &code = INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT) {
   ...
   action_result set_tx_fee(const string &operation, const string &fee) {
      return push_action(N(settxfee), mvo()
            ("operation", operation)
            ("fee", fee), config::system_account_name, abi_ser_txfee, INFRABLOCKCHAIN_SYS_TX_FEE_ACCOUNT);
   }
   ...
   set_id_info(account_name account, account_name idauth, uint16_t type, uint16_t kyc, uint32_t state,
      ...
      try {
         base_tester::push_action(INFRABLOCKCHAIN_SYS_IDENTITY_ACCOUNT, N(setidinfo), idauth, mvo()
               ("account", account)
               ("identity_authority", idauth)
               ("type", type)
               ("kyc", kyc)
   ...
   action_result setkyc(account_name account, uint16_t kyc, account_name idauth) {
      try {
         base_tester::push_action(INFRABLOCKCHAIN_SYS_IDENTITY_ACCOUNT, N(setkyc), idauth, mvo()
               ("account", account)
               ("kyc", kyc));
         return success();
      } catch (const fc::exception &ex) {
   ...
   fc::variant ntoken_get_stats(const account_name &depo) {
      vector<char> data = get_row_by_account(INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT, depo, N(ntstats), N(basicstats));
   ...
   fc::variant ntoken_get_accounts(const account_name &acc, const account_name &depo) {
      vector<char> data = get_row_by_account(INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT, acc, N(ntaccounts), depo);
   ...
   fc::variant ntoken_get_accounts_total(const account_name &acc) {
      vector<char> data = get_row_by_account(INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT, acc, N(ntaccountstt), N(totalbal));
   ...
   transaction_trace_ptr nredeem(const string &token, const string &memo) {
      auto _token = yx_asset::from_string(token);
      return base_tester::push_action(INFRABLOCKCHAIN_SYS_NATIVE_TOKEN_ACCOUNT, N(nredeem), _token.issuer, mvo()
            ("token", _token)
            ("memo", memo));
   }
   ...
   fc::variant token_get_stats(int64_t token_symbol, const account_name &issuer) {
      vector<char> data = get_row_by_account(INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT, token_symbol, N(tstats), issuer);
   ...
   fc::variant token_get_accounts(const account_name &owner, const account_name &issuer) {
      vector<char> data = get_row_by_account(INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT, owner, N(taccounts), issuer);
   ...
   action_result token_set_kyc_rule(const string &ysymbol, uint8_t type, uint16_t kyc_flags) {
      ...
      return push_action(N(setkycrule), mvo()
            ("ysymbol", _ysymbol)
            ("type", type)
            ("kyc", kyc_flags), _ysymbol.issuer, abi_ser_token, INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT);
   }
   ...
   action_result token_set_options(const string &ysymbol, uint16_t options, bool reset) {
      ...
      return push_action(N(setoptions), mvo()
                               ...
                               ("options", options)
                               ("reset", reset),
                         _ysymbol.issuer, abi_ser_token, INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT);
   }
   ...
   action_result token_issue(const account_name &to, const string &token, const string &memo) {
      ...
      return push_action(N(issue), mvo()
            ("to", to)
            ("token", _token)
            ("memo", memo), _token.issuer, abi_ser_token, INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT);
   }

   action_result token_grantissue(const account_name &to, const string &token) {
      auto _token = yx_asset::from_string(token);
      return push_action(N(grantissue), mvo()
            ("to", to)
            ("token", _token), _token.issuer, abi_ser_token, INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT);
   }
   ...
   action_result token_issuebyuser(const account_name &user, const account_name &to, const string &token, const string &memo) {
      ...
      return push_action(N(issuebyuser), mvo()
            ...
            ("to", to)
            ("token", _token)
            ("memo", memo), user, abi_ser_token, INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT);
   }
   ...
   action_result token_changeissued(const account_name &user, const string &delta, bool decrease) {
      ...
      return push_action(N(changeissued), mvo()
            ("user", user)
            ("delta", _delta)
            ("decrease", decrease), _delta.issuer, abi_ser_token, INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT);
   }

   transaction_trace_ptr token_redeem(const string &token, const string &memo) {
      auto _token = yx_asset::from_string(token);
      return base_tester::push_action(INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT, N(redeem), _token.issuer, mvo()
            ("token", _token)
            ("memo", memo));
   }
   ...
   action_result token_transfer(account_name from, account_name to, const string &token, const string &memo) {
      ...
      return push_action(N(transfer), mvo()
            ...
            ("to", to)
            ("token", _token)
            ("memo", memo), from, abi_ser_token, INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT);
   }
   ...
   token_wptransfer(account_name from, account_name to, const string &token, account_name payer, const string &memo) {
      try {
         auto _token = yx_asset::from_string(token);
         base_tester::push_action(INFRABLOCKCHAIN_SYS_USER_TOKEN_ACCOUNT, N(wptransfer),
   ...
   fc::variant nft_get_stats(int64_t token_symbol, const account_name &issuer) {
      vector<char> data = get_row_by_account(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT, token_symbol, N(tstats), issuer);
   ...
   fc::variant nft_get_accounts(const account_name &owner, const account_name &issuer) {
      vector<char> data = get_row_by_account(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT, owner, N(taccounts), issuer);
   ...
   fc::variant nft_get_token(id_type token_id, account_name issuer) {
      vector<char> data = get_row_by_account(INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT, issuer, N(nftokens), token_id);
   ...
   action_result nft_set_kyc_rule(const string &ysymbol, uint8_t type, uint16_t kyc_flags) {
      ...
      return push_action(N(setkycrule), mvo()
            ("ysymbol", _ysymbol)
            ("type", type)
            ("kyc", kyc_flags), _ysymbol.issuer, abi_ser_nft, INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT);
   }
   ...
   action_result nft_set_options(const string &ysymbol, uint16_t options, bool reset) {
      ...
      return push_action(N(setoptions), mvo()
                               ...
                               ("options", options)
                               ("reset", reset),
                         _ysymbol.issuer, abi_ser_nft, INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT);
   }
   ...
   nft_transferid(account_name from, account_name to, account_name issuer, const T &ids, const string &memo) {
      return push_action(N(transferid), mvo()
                               ...
                               ("ids", ids)
                               ("memo", memo),
                         from, abi_ser_nft, INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT
      );
   }
   ...
   action_result nft_redeem(account_name issuer, const T &ids, const string &memo) {
      return push_action(N(redeem), mvo()
                               ...
                               ("ids", ids)
                               ("memo", memo),
                         issuer, abi_ser_nft, INFRABLOCKCHAIN_SYS_NON_FUNGIBLE_TOKEN_ACCOUNT
      );
   }
};

}} // namespace infrablockchain::testing

#endif //INFRABLOCKCHAIN_SYS_TESTER_HPP
