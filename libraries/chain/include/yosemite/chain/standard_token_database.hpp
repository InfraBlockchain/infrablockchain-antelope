/**
 *  @file yosemite/chain/standard_token_database.hpp
...
namespace yosemite { namespace chain {
    ...
    */
   class token_balance_object : public chainbase::object<yosemite_token_balance_object_type, token_balance_object> {
...
namespace eosio { namespace chain { namespace config {
   ...
   template<>
   struct billable_size<yosemite::chain::token_balance_object> {
      static const uint64_t overhead = overhead_per_row_per_index_ram_bytes * 2;  ///< overhead for 2x indices internal-key(id) and token_id
