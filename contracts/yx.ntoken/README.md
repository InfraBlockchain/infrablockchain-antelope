...
* The native token must be a FIAT-token, pegged to 1:1 exchange rate. This is designed to be used as the network's transaction fee and the incentive for the block producers.
   * Only the system depositories issue and redeem the native token. For more information, refer to [infrasys](../../contracts/infrasys/).
   * Only the blockchain users who have successfully verified their identity by completing the KYC process can transfer the native token. For more information, refer to [sys.identity](../../contracts/sys.identity/).
...
* For the native token, YosemiteChain defines its symbol and precision at the Yosemite software release to conform to such perspective.
   ...
   * There would be the Yosemite software with 2,DUSD or 2,EUR. These softwares can be made with CMake build options, INFRABLOCKCHAIN_NATIVE_TOKEN_SYMBOL_NAME and INFRABLOCKCHAIN_NATIVE_TOKEN_PRECISION. Refer to [root CMakeLists.txt](../CMakeLists.txt) file.
...
* YosemiteChain allows multiple issuers for the native token. Native token issuer, which is named to the system depository, is registered by [infrasys](../../contracts/infrasys/).
...
* Only system depositories can issue and redeem the native token.
  ...
  * If a new depository wants to issue the native token, it must call [infrasys](../../contracts/infrasys/)::regsysdepo action first to become the system depository.
...
```
infra-cli push action sys.txfee settxfee '{"operation":"tf.nissue", "fee":"0.00 DKRW"}}' -p yosemite
infra-cli push action sys.txfee settxfee '{"operation":"tf.nredeem", "fee":"1000.00 DKRW"}}' -p yosemite
infra-cli push action sys.txfee settxfee '{"operation":"tf.ntransfer", "fee":"10.00 DKRW"}}' -p yosemite
infra-cli push action sys.txfee settxfee '{"operation":"tf.transfer", "fee":"20.00 DKRW"}}' -p yosemite
```
...
1. type
   * 0 : transfer send
   * 1 : transfer receive
1. kyc : 16-bit flags from [sys.identity](../../contracts/sys.identity/)
...
* d1 must be the system depository registered and authroized by [infrasys](../../contracts/infrasys/).
...
* The `payfee` action is notified to the payer acount and sys.txfee account.
```
...
#      sys.txfee <= yx.ntoken::payfee            {"payer":"d1","token":{"amount":"1000.00 DKRW","issuer":"d1"}}
```
...
#      sys.txfee <= yx.ntoken::payfee            {"payer":"user1","token":{"amount":"20.00 DKRW","issuer":"d1"}}
```
...
#      sys.txfee <= yx.ntoken::payfee            {"payer":"user1","token":{"amount":"20.00 DKRW","issuer":"d2"}}
```
