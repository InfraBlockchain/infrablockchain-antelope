/**
 *  @file contracts/infrasys/infrasys.block_producer.cpp
...
//    const uint64_t useconds_per_year     = seconds_per_year*1000000ll;
    ...
    void system_contract::update_elected_producers( block_timestamp block_time ) {
       ...
       top_producers.reserve( INFRABLOCKCHAIN_MAX_ELECTED_BLOCK_PRODUCER_COUNT );
       ...
       while ( fetched_list_packed_size > 0 /*&& top_producers.size() < INFRABLOCKCHAIN_MAX_ELECTED_BLOCK_PRODUCER_COUNT*/ ) {
          uint32_t fetched_cnt = fetched_list_packed_size / sizeof(struct infrablockchain_tx_vote_stat_for_account);
          ...
          infrablockchain_tx_vote_stat_for_account *tx_vote_receiver_stat_arr = reinterpret_cast<struct infrablockchain_tx_vote_stat_for_account *>(buf_top_tx_vote_receivers_data_packed);

          for (uint32_t i = 0; i < fetched_cnt; i++) {
             infrablockchain_tx_vote_stat_for_account &tx_vote_stat_for_account = tx_vote_receiver_stat_arr[i];

             auto producer_itr = _producers.find( tx_vote_stat_for_account.account );
             if (producer_itr != _producers.cend()) {
                if ( top_producers.size() < INFRABLOCKCHAIN_MAX_ELECTED_BLOCK_PRODUCER_COUNT && producer_itr->active() && producer_itr->is_trusted_seed ) {
                   ...
                   if ( top_producers.size() == INFRABLOCKCHAIN_MAX_ELECTED_BLOCK_PRODUCER_COUNT ) {
                      break;
                   }
                }
             }
          ...
          if ( top_producers.size() < INFRABLOCKCHAIN_MAX_ELECTED_BLOCK_PRODUCER_COUNT && fetched_cnt == top_tx_vote_receiver_list_fetch_count ) {
