/**
 *  @file contracts/infrasys/infrasys.system_token.cpp
...
namespace infrablockchainsys {
   ...
   void system_contract::addsystoken( const account_name token, const uint32_t weight ) {
      ...
      vector<infrablockchain_system_token> new_system_token_list;
      ...
      if ( sys_token_count > 0 ) {
         ...
         if ( sys_token_list_packed_size > 0 ) {
            ...
            vector<infrablockchain_system_token> current_system_tokens;
            ...
            for ( const auto& sys_token : current_system_tokens ) {
               if ( sys_token.token_id == token ) {
                  ...
                  new_system_token_list.push_back( infrablockchain_system_token{token, weight} );
               } else {
