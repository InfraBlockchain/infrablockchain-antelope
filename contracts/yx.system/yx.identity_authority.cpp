    ...
    void system_contract::regidauth( const account_name identity_authority, const std::string& url, uint16_t location ) {
        ...
        if (!has_auth(_self)) {
            INLINE_ACTION_SENDER(yosemite::ntoken, transfer)
                    (N(yx.ntoken), {identity_authority, N(active)},
                     { identity_authority, YOSEMITE_SYSTEM_ACCOUNT_NAME, yx_asset{YOSEMITE_REG_ID_AUTH_DEPO_TX_FEE, 0}, identity_authority, "regidauth" });
        }
    }
