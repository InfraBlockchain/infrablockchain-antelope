...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.cli + 'create account eosio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.cli + 'system newaccount --transfer eosio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' %
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.cli + 'transfer eosio %s "%s"' % (a['name'], intToCurrency(unstaked)))
...
def claimRewards():
    table = getJsonOutput(args.cli + 'get table eosio eosio producers -l 100')
    times = []
...
def updateAuth(account, permission, parent, controller):
    run(args.cli + 'push action eosio updateauth' + jsonArg({
        'account': account,
...
def msigReplaceSystem():
    run(args.cli + 'push action eosio buyrambytes' + jsonArg(['eosio', accounts[0]['name'], 200000]) + '-p eosio')
    sleep(1)
...
def stepInstallSystemContracts():
    run(args.cli + 'set contract eosio.token ' + args.contracts_dir + '/eosio.token/')
    run(args.cli + 'set contract eosio.msig ' + args.contracts_dir + '/eosio.msig/')
def stepCreateTokens():
    run(args.cli + 'push action eosio.token create \'["eosio", "10000000000.0000 %s"]\' -p eosio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.cli + 'push action eosio.token issue \'["eosio", "%s", "memo"]\' -p eosio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.cli + 'set contract eosio ' + args.contracts_dir + '/eosio.system/')
    sleep(1)
    run(args.cli + 'push action eosio setpriv' + jsonArg(['eosio.msig', 1]) + '-p eosio@active')
def stepInitSystemContract():
    run(args.cli + 'push action eosio init' + jsonArg(['0', '4,SYS']) + '-p eosio@active')
    sleep(1)
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for infra-cli')
