...
public:
   ...
   eosio_sudo_tester() {
      ...
      while( control->pending_block_state()->header.producer.to_string() == "yosemite" ) {
         produce_block();
      }
   }
...
BOOST_FIXTURE_TEST_CASE( sudo_exec_direct, eosio_sudo_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( "yosemite", name{trace->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig, eosio_sudo_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( "yosemite", name{traces[1]->action_traces[0].act.account} );
